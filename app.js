...
// ========================================
async function handleFileUpload(categoryType, inputElement) {
  console.log('📤 handleFileUpload llamado:', categoryType);
  
  // CRÍTICO: Guardar el tipo en una variable local
  ...
  console.log(`📷 ${files.length} archivo(s) seleccionado(s) para ${type}`);
  
  // Validar límite según modo
  let maxFiles;
  if (closetMode) {
    ...
    if (currentCount + files.length > maxFiles) {
      showNotification(`Máximo ${maxFiles} ${type} permitidas`, 'error');
      return;
    }
  }
  ...
  for (const file of files) {
    try {
      ...
      if (!detection.success || detection.category === 'unknown') {
        showNotification(`❌ ${file.name} no es una prenda válida`, 'error');
        continue;
      }
  ...
  if (successCount > 0) {
    ...
    // Si estamos en closet, re-renderizar
    if (closetMode) {
    ...
    showNotification(`✅ ${successCount} prenda(s) agregada(s)! ${closetMode ? `Armario: ${total}/${CONFIG.TOTAL_CLOSET_LIMIT} (${remaining} restantes)` : ''}`, 'success');
  } else {
    showNotification('❌ No se pudo agregar ninguna prenda', 'error');
  }
